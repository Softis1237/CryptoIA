...
`chart_vision_agent.py` обрабатывает изображения графиков по нескольким таймфремам и возвращает структурированный анализ (bias + confidence). При включении генерации графиков агент получает ссылки из S3 и подготавливает вход для мультимодальных LLM.
...
`agents/red_team_agent.py` строит стресс-сценарии на основе уроков памяти и публикует рекомендации по улучшению стратегий. Агент вызывается оркестратором в High-Alert режиме, фиксирует количество сценариев в метриках и записывает гипотезы для последующего бэктеста.
