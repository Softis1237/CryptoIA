...
Новая архитектура аналитического совета описана в [ANALYST_ROADMAP_V3.md](docs/ANALYST_ROADMAP_V3.md). Настройка промптов/контроль LLM — [ANALYST_TUNING_GUIDE.md](docs/ANALYST_TUNING_GUIDE.md), [LLM_FAILSAFE.md](docs/LLM_FAILSAFE.md). Процедуры бэктеста и paper/live торговли: [BACKTEST_RUNBOOK.md](docs/BACKTEST_RUNBOOK.md), [PAPER_TRADING_RUNBOOK.md](docs/PAPER_TRADING_RUNBOOK.md), [LIVE_TRADING_CHECKLIST.md](docs/LIVE_TRADING_CHECKLIST.md). Общий гайд по логированию — [LOGGING_GUIDE.md](docs/LOGGING_GUIDE.md). Оптимизация/профилирование и метрики — [PERFORMANCE_OPTIMIZATION.md](docs/PERFORMANCE_OPTIMIZATION.md) и `docs/OBSERVABILITY.md`.

Новая дорожная карта развития v2 с прогрессом и параметрами RT‑триггеров: [docs/ROADMAP_V2.md](docs/ROADMAP_V2.md).

Основные новшества (ядро торговли и аналитики):
- Векторный бэктестер с симулятором биржи и CLI (`python -m pipeline.cli.backtest`).
- Портфельный менеджер и политики (масштабирование позиции, запрет реверса при открытой позиции). Включение: `PORTFOLIO_ENABLED=1`.
- Детекторы SMC‑зон (Order Blocks, FVG, Liquidity Pools, Breaker) и оверлей на графиках.
- PostMortemAgent: автоматический разбор закрытых сделок и запись уроков в `agent_lessons`.
- InvestmentArbiter учитывает доверие к моделям и структурированные уроки, динамически перенастраивая веса агентов и фиксируя источник ATR в карточке сделки.
- ML-бандл обучается сразу на нескольких горизонтах (`30m`, `4h`, `12h`, `24h`); команды `retrain`/Windmill сохраняют единый артефакт с картой метрик.
- TradeRecommend и оптимизатор риска используют ATR соответствующего горизонта, автоматически масштабируя стоп/плечо для коротких и длительных сценариев.
- ContextBuilderAgent формирует оперативную память (макро, ончейн, уроки, TA) с кешированием и ограничением по токенам.
- InvestmentArbiter v2 строит Chain-of-Thought на основе оперативной памяти и передает выводы SelfCritiqueAgent, который корректирует вероятность и фиксирует контраргументы.
Поток релиза
...
Расширенные возможности
    ...
    • Векторный бэктест: `python -m pipeline.cli.backtest <prices.csv> --strategy pipeline.trading.backtest.strategies:MovingAverageCrossStrategy` запускает симуляцию биржи с учётом комиссий, проскальзывания и частичных исполнений и формирует JSON-отчёт о метриках.
...
• Всегда устанавливайте стоп‑лосс и не храните крупные суммы на торговых счетах.
    • Храните ключи API и секреты в специализированном хранилище (Vault/SOPS), регулярно делайте бэкапы Postgres/S3. См. `docs/OPS_SECURITY.md`.
    • Для пользователей Telegram-бота предусмотрите обязательное подтверждение юридического дисклеймера перед получением сигналов.
